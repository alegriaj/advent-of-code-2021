...
// output - score of board that wins last
// Time: O(n^5)
// Space: O(nm)
function getLastWinningScore(filename) {
  const input = getArrayStringsFromFile(filename);

  const drawOrder = getDrawOrder(input);
  const boards = getBoards(input);
  const losers = new Set();
  boards.forEach((_, i) => losers.add(i));

  const [winningBoard, winningDraw] = doDraws(drawOrder, boards, losers);
...
// Time: O(n^4)
// Space: O(1)
function applyDrawToBoards(draw, boards, losers) {
...
// Time: O(n^5)
// Space: O(1)
function doDraws(drawOrder, boards, losers) {
  let loser = -1;
  for (draw of drawOrder) {
    applyDrawToBoards(draw, boards, losers);

    switch ([...losers].length) {
      case 1: // this will be the loser, but hasn't 'won' yet
        loser = [...losers];
        break;
      case 0: // now they 'won'
        return [loser, draw];
